        ...
        if(type == 'datasource') {
            dbObj.getSchemas(function(data) {
                for(var i = 0; i < data.length; i++) {
                    ...
                    array.push({id: id, name: schema.name, type: type, children: [
                        ...
                        {id: id, name: '函数', type: 'function', isParent: true}
                    ]});
                }
                callback(array);
            })
            ...
            dbObj.getFunctions(pids[1], function(data) {
                for(var i = 0; i < data.length; i++) {
                    ...
                    array.push({id: id, name: func.name, displayName: func.displayName, type: type, children: [{id: id, name: '参数', type: 'param', isParent: true}]});
                }
                callback(array);
            })
        } else if(type == 'column') {

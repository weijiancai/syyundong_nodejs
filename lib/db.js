    ...
    if(!type || !id) {
        async.parallel({
            ...
            schema: function(callback) {
                dbObj.query(dbObj.getSchemaSearchSql(value), function(data) {
                    if(data && data.length > 0) {
                        for(var i = 0; i < data.length; i++) {
                            var schema = data[i];
                            toResult(schema.name, '/public/images/db/Schema.png', dataSource.name, schema.id, schema.comment, schema.dbType);
                        }
                    }
                    callback();
                });

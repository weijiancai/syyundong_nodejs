...
function toMetaConfig(dbId, callback) {
    ...
    dbObj.getColumns(pids[1], pids[2], function(data) {
        ...
        for(var i = 0; i < data.length; i++) {
            ...
            var obj = {id: id, name: column.name, displayName: column.displayName || column.name, dataType: dataType, isDisplay: true, dict: '', align: 'left', sortNum: (i+1), isValid: true, maxLength: maxLength, displayStyle: 0};
            obj.isPk = column['isPrimaryKey'] == 'Y';
            obj.isFk = column['isForeignKey'] == 'Y';

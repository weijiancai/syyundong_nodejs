...
function getSearchSql(value, schema) {
    var sql = '';
    if(!schema) {
    ...
    var columnSql = sqlBuilder.create()
        .query("col.TABLE_SCHEMA + '.' + col.TABLE_NAME id, col.COLUMN_NAME name , col.COLUMN_COMMENT comment,")
        .query("(case when (col.COLUMN_KEY = 'PRI' and kcu.COLUMN_NAME is not null) then 'PKFKCOLUMN' when (col.COLUMN_KEY = 'PRI') then 'PKCOLUMN' when (kcu.COLUMN_NAME is not null) then 'FKCOLUMN' else 'COLUMN' end) dbType")
        ...
        .order("col.ORDINAL_POSITION asc")
        .build();

    var constraintSql = sqlBuilder.create()
        .query("tc.TABLE_SCHEMA + '.' + tc.TABLE_NAME + '.' + tc.CONSTRAINT_NAME id, tc.CONSTRAINT_NAME name, '' comment, 'CONSTRAINT' dbType")
        .from("INFORMATION_SCHEMA.TABLE_CONSTRAINTS tc")
        .where()
        .ands("tc.CONSTRAINT_TYPE <> 'PRIMARY KEY' and tc.CONSTRAINT_TYPE <> 'UNIQUE'")
        .like("tc.CONSTRAINT_NAME", value)
        .order("tc.TABLE_NAME, tc.CONSTRAINT_NAME asc")
        .build();

    var indexSql = sqlBuilder.create()
        .query("distinct (TABLE_SCHEMA + '.' + TABLE_NAME + '.' + COLUMN_NAME) id, INDEX_NAME name, '' comment, 'INDEX' dbType")
        .from("INFORMATION_SCHEMA.STATISTICS")
        .where()
        .like("index_name", value)
        .order("TABLE_NAME, INDEX_NAME asc")
        .build();

    var triggerSql = sqlBuilder.create()
        .query("EVENT_OBJECT_SCHEMA + '.' + EVENT_OBJECT_TABLE id, TRIGGER_NAME name, '' comment, 'TRIGGER' dbType")
        .from("INFORMATION_SCHEMA.TRIGGERS")
        .where()
        .like("TRIGGER_NAME", value)
        .order("EVENT_OBJECT_TABLE, TRIGGER_NAME asc")
        .build();

...
function queryByPage(sql, start, length, callback, tableName, pkColName) {
    sql = sql.toLowerCase();
    if(_dataSource.isVpn) {
        vpnQueryByPage(sql, start, length, callback, tableName, pkColName);
        return;
    }
    ...
    async.parallel({
        ...
        rows: function(callback) {
            sql = getPageSql(sql, tableName, pkColName, start, length);
            console.log(sql);
            connection = new Connection(config);
            connection.on('connect', function(err) {
                    if(err) {
...
function getPageSql(sql, tableName, pkColName, start, length) {
    if(tableName && pkColName) {
...
function vpnQueryByPage(sql, start, length, callback, tableName, pkColName) {
    var countSql = getCountSql(sql);
    var pageSql = getPageSql(sql, tableName, pkColName, start, length);
    var request = require('request');
...
function getSearchSql(value, schemas, filter) {
    ...
    for(var i = 0; i < schemas.length; i++) {
        ...
        if(filter) {
            ...
            } else if(underscore._.indexOf(array, 'VIEW') > -1) {
                objSql.ands("type in ('V')");
            } else if(underscore._.indexOf(array, 'VIEW') > -1) {
                objSql.ands("type in ('V')");
            } else if(underscore._.indexOf(array, 'CONSTRAINT') > -1 || underscore._.indexOf(array, 'INDEX') > -1) {
                objSql.ands("type in ('F', 'PK', 'UQ')");
            } else if(underscore._.indexOf(array, 'TRIGGER') > -1) {
                objSql.ands("type in ('TA', 'TR')");
            } else if(underscore._.indexOf(array, 'PROCEDURE') > -1) {
                objSql.ands("type in ('P', 'PC', 'X')");
            } else if(underscore._.indexOf(array, 'FUNCTION') > -1) {
